...
<head>
    ...
    <style>
        /* ============================================================
         |  PAGE SETUP — A4 LANDSCAPE, 1 risiko = 1 halaman
         ============================================================ */
        @page {
            size: A4 landscape;
            margin-top: 15mm;
...
<body>
    ...
    @php
        /* Logo di-embed sebagai base64 agar dompdf bisa memuatnya */
        $logoPath = public_path('img/logo kop polinema.png');
        $logoBase64 = '';
        if (file_exists($logoPath)) {
            $logoBase64 = 'data:image/png;base64,' . base64_encode(file_get_contents($logoPath));
        }
        $total = count($petaItems);
    @endphp
                ...
                <tbody>
                    @if ($hasilAudit)
                        <tr>
                            ...
                            <td class="col-mitigasi"
                                style="min-height:130px; text-align:center; vertical-align:middle;">
                                @php
                            ...
                            <td class="col-komentar" style="min-height:130px;">
                                ...
                                @if (count($baris) > 0)
                                    @foreach ($baris as $b)
                                        @php
                                            // Pisahkan "1. Judul: teks" → nomor, judul, teks
                                            // Pola: "1. Judul Kata: sisa teks" atau "1. Teks biasa"
                                            $noStr = '';
                                            $judulStr = '';
                                            $sisaStr = $b;
                                            if (preg_match('/^(\d+\.\s*)(.+)$/', $b, $m)) {
                                                $noStr = rtrim($m[1]);
                                                $konten = $m[2];
                                                // Ada judul jika ada titik dua sebelum spasi panjang
