...
<head>
    ...
    <style>
        ...
        /* ── PAGE BREAK: Setiap risiko di halaman baru ── */
        .halaman-risiko {
            page-break-after: always;
        }

        /* Halaman terakhir tidak perlu page-break */
        .halaman-risiko:last-child {
...
<body>
    ...
    @php $total = count($petaItems); @endphp

    @foreach ($petaItems as $idx => $item)
        @php
            $peta = $item['peta'];
            $hasilAudit = $item['hasilAudit'];
            $skorTotal = $item['skorTotal'];
            $levelText = $item['levelText'];
            $residualText = $item['residualText'];
            $halamanKe = $idx + 1;
        @endphp

        <div class="halaman-risiko">

            {{-- ── NOMOR HALAMAN ── --}}
            <div class="page-info">
                Halaman {{ $halamanKe }} dari {{ $total }} |
            ...
            <table>
                ...
                <tr>
                    ...
                    <td width="39.5%">
                        <span class="value">: {{ $hasilAudit->nama_pemonev ?? ($user->name ?? '-') }}</span>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td class="no-border-middle"></td>
                    <td colspan="2">
                        <span class="label">Tahun Anggaran KEGIATAN</span>
                        <span class="value">: {{ $hasilAudit->tahun_anggaran ?? $tahun }}</span>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td colspan="4">
                        <span class="value">:
                            {{ $hasilAudit->kegiatan ?? ($peta->kegiatan->judul ?? $peta->judul) }}</span>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td colspan="4">
                        <span class="value">: {{ $peta->pernyataan ?? '-' }}</span>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        @php
                            $lv = $hasilAudit->level_risiko ?? $levelText;
                            $lvClass = match ($lv) {
                        ...
                        @endphp
                        <span class="{{ $lvClass }}">{{ $lv }}</span>
                    </td>
                    <td class="no-border-middle"></td>
                    <td class="label">RISIKO RESIDUAL</td>
                    <td>
                        @php
                            $rv = $hasilAudit->risiko_residual ?? $residualText;
                            $rvClass = match ($rv) {
                                'Extreme' => 'residual-extreme',
                                'High' => 'residual-high',
                                'Moderate' => 'residual-moderate',
                                default => 'residual-low',
                            };
                        @endphp
                        <span class="{{ $rvClass }}">{{ $rv }}</span>
                    </td>
                </tr>

                {{-- Baris 6: PENGENDALIAN | MITIGASI | KOMENTAR --}}
                @if ($hasilAudit)
                    <tr class="header-row">
                        ...
                        <td colspan="2" class="table-content" style="height: 180px; vertical-align: top;">
                            ...
                            @if ($peta->status_konfirmasi_auditee || $peta->status_konfirmasi_auditor)
                                ...
                                @if ($peta->status_konfirmasi_auditee)
                                    <strong>Auditee:</strong> {{ $peta->status_konfirmasi_auditee }}<br>
                                @endif
                                @if ($peta->status_konfirmasi_auditor)
                                    <strong>Auditor:</strong> {{ $peta->status_konfirmasi_auditor }}
                                @endif
                            @endif
                        </td>
                ...
                @else
                    <tr>
                        <td colspan="5" style="text-align: center; padding: 15px; color: #999; height: 180px;">
                            <em>Data audit belum diisi oleh Auditor</em>
                        </td>
                    </tr>
                @endif
            </table>
            ...
            <div class="signature-section">
                <div class="signature-container">
                    <div class="signature-left">
                        ...
                        @else
                            <span>NIP. ___________________________</span>
                        @endif
                    </div>
                    <div class="signature-right">
                        ...
                        <span>NIP. {{ $hasilAudit->nip_pemonev ?? ($user->nip ?? '-') }}</span>
                    </div>
                </div>
            </div>

        </div>{{-- end .halaman-risiko --}}
    @endforeach
